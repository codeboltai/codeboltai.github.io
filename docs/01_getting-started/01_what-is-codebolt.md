...
5. **Multi-agent is first-class.** Swarms, agent flows, roles, teams, plan-execute-review, debate, stigmergy — all supported, with a graph runtime and a full event log. See [Orchestration Design](../04_build-on-codebolt/07_orchestration-design/01_overview.md).
...
- **Custom agents.** Build an agent specialised for your codebase — your coding conventions, your internal APIs, your review standards. See [Build Agents](../04_build-on-codebolt/02_custom-agents/01_overview.md).
- **Custom tools.** Write an MCP server that gives agents access to your internal systems. See [Custom Tools & MCP](../04_build-on-codebolt/04_custom-tools-and-mcp/01_overview.md).
- **Multi-agent workflows.** Set up a plan-execute-review flow for high-risk changes, or a map-reduce for embarrassingly parallel work. See [Orchestration Design](../04_build-on-codebolt/07_orchestration-design/01_overview.md).
- **Self-hosting for a team.** Run the server yourself, with shared memory, guardrails, and audit trails across your organisation. See [Self-Hosting](../04_build-on-codebolt/09_self-hosting/01_overview.md).
