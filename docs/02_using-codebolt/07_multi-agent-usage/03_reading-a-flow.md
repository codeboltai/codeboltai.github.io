...
The flow definition lives in YAML (see [Agent Flows](../../04_build-on-codebolt/07_orchestration-design/04_agent-flows.md)). Use `codebolt flow show <flow-name>` (CLI) or **Flows panel → Definition** (desktop) to see the YAML — useful for understanding "why did the flow route to node X?" by checking the edge conditions.
...
- [Agent Flows](../../04_build-on-codebolt/07_orchestration-design/04_agent-flows.md)
