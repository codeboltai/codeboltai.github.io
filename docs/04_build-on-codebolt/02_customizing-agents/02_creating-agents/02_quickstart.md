...
Note that this agent has **read tools only** and declares a structured output — it's designed to be the "plan" stage of a [plan-execute-review](../07_orchestration-design/03_patterns/plan-execute-review.md) pipeline, not a standalone chat agent.
...
`createCodeboltAgent` is the [Unified Agent](./05_patterns/unified-agent.md) pattern — it handles the loop, the context, the heartbeats, everything. You provide the `run` handler.
...
Now that the agent has a structured output, you can use it as a node in an [agent flow](../07_orchestration-design/04_agent-flows.md):
...
- **Pick a pattern for non-trivial agents** → [Patterns overview](./05_patterns/overview.md)
- **Add custom behaviour via processors** → [Processors](../03_processors/01_what-are-processors.md)
- **Test your agent rigorously** → [Testing and debugging](./06_testing-and-debugging.md)
- **Ship it** → [Publishing](./07_publishing.md)
...
- **Re-implementing planning.** The [planning hierarchy](../08_internals/03_subsystems/08_planning-hierarchy.md) already exists. Use it instead of rolling your own.
- **No tests.** An agent without replay tests will silently regress when you change its prompt. Set up [testing](./06_testing-and-debugging.md) from day one.
