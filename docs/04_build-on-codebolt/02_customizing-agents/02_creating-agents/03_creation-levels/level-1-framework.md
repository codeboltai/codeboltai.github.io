...
| **Context assembly** | Default [contextAssembly](../../08_internals/03_subsystems/07_context-assembly.md) wiring, configurable |
| **Tool calling** | Typed access to the [tool service](../../08_internals/03_subsystems/02_mcp-and-tools.md) |
| **LLM client** | Typed access to [llmService](../../08_internals/03_subsystems/03_llm-and-inference.md) |
| **Memory access** | Typed access to [memory layers](../../08_internals/03_subsystems/04_memory.md) |
...
The framework wraps this in the [agent loop](../04_agent-anatomy/lifecycle.md), handles retries, and records every phase.

## Using a pattern

For anything beyond the trivial, use one of the [agent patterns](../05_patterns/overview.md) instead of writing the loop yourself:
...
[Unified Agent](../05_patterns/unified-agent.md) is the recommended default. It packages the common agent shape with sensible defaults and still lets you customize every phase.
...
See [Testing and debugging](../06_testing-and-debugging.md) for the full surface.
...
- [Patterns overview](../05_patterns/overview.md)
- [codeboltagent.yaml reference](../04_agent-anatomy/agent-yaml.md)
