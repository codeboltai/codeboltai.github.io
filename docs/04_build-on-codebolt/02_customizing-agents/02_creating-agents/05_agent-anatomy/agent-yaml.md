...
### `processors`
Optional. Lets you attach processors at specific slots in the loop. If you're using a pattern ([Unified](../05_patterns/unified-agent.md), [Composable](../05_patterns/composable-pattern.md), etc.), the pattern often wires these for you.
### `inputs`, `outputs`
...
### `triggers`
For [background agents](../../08_internals/03_subsystems/01_agent-subsystem.md) that run on schedules or events instead of chat. Each trigger type is documented under its own page (cron, file-change, webhook).
### `remix` block
