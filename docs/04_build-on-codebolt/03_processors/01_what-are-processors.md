...
| **[Hooks](../06_hooks-and-capabilities/01_hooks-overview.md)** | Events on the application bus | Events (not tied to a specific agent loop) | Cross-cutting concerns: logging, auditing, policy enforcement |
| **[Tools](../04_custom-tools-and-mcp/01_overview.md)** | When the LLM calls them | A named capability | New things the agent can do |
...
- [Context Assembly internals](../08_internals/03_subsystems/07_context-assembly.md) — where many processors run
