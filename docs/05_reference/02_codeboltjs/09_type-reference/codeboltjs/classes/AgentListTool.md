...
Defined in: packages/codeboltjs/src/tools/agent/agent-list.ts:104

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`AgentListParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new AgentListTool(): AgentListTool;
```

Defined in: packages/codeboltjs/src/tools/agent/agent-list.ts:107
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'agent_list'` | - | - | packages/codeboltjs/src/tools/agent/agent-list.ts:105 |
...
```ts
build(params: AgentListParams): ToolInvocation<AgentListParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`AgentListParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: AgentListParams): ToolInvocation<AgentListParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/agent/agent-list.ts:126
...
```ts
validateBuildAndExecute(params: AgentListParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: AgentListParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: AgentListParams): string | null;
```
