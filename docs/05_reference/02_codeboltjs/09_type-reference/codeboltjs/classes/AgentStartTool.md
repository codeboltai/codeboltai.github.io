...
Defined in: packages/codeboltjs/src/tools/agent/agent-start.ts:73

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`AgentStartParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new AgentStartTool(): AgentStartTool;
```

Defined in: packages/codeboltjs/src/tools/agent/agent-start.ts:76
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'agent_start'` | - | - | packages/codeboltjs/src/tools/agent/agent-start.ts:74 |
...
```ts
build(params: AgentStartParams): ToolInvocation<AgentStartParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`AgentStartParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: AgentStartParams): ToolInvocation<AgentStartParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/agent/agent-start.ts:113
...
```ts
validateBuildAndExecute(params: AgentStartParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: AgentStartParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/agent/agent-start.ts:103
