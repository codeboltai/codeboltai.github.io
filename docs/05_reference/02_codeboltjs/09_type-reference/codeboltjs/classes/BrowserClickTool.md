...
Defined in: packages/codeboltjs/src/tools/browser/click.ts:66

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserClickParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserClickTool(): BrowserClickTool;
```

Defined in: packages/codeboltjs/src/tools/browser/click.ts:69
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_click'` | - | - | packages/codeboltjs/src/tools/browser/click.ts:67 |
...
```ts
build(params: BrowserClickParams): ToolInvocation<BrowserClickParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserClickParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserClickParams): ToolInvocation<BrowserClickParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/click.ts:103
...
```ts
validateBuildAndExecute(params: BrowserClickParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserClickParams): string | null;
```
...
```ts
protected validateToolParamValues(params: BrowserClickParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/browser/click.ts:96
