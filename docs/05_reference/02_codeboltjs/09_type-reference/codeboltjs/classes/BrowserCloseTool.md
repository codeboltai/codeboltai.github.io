...
Defined in: packages/codeboltjs/src/tools/browser/close.ts:50

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserCloseParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserCloseTool(): BrowserCloseTool;
```

Defined in: packages/codeboltjs/src/tools/browser/close.ts:53
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_close'` | - | - | packages/codeboltjs/src/tools/browser/close.ts:51 |
...
```ts
build(params: BrowserCloseParams): ToolInvocation<BrowserCloseParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserCloseParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserCloseParams): ToolInvocation<BrowserCloseParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/close.ts:76
...
```ts
validateBuildAndExecute(params: BrowserCloseParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserCloseParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: BrowserCloseParams): string | null;
```
