...
Defined in: packages/codeboltjs/src/tools/browser/get-content.ts:65

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserGetContentParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserGetContentTool(): BrowserGetContentTool;
```

Defined in: packages/codeboltjs/src/tools/browser/get-content.ts:68
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_get_content'` | - | - | packages/codeboltjs/src/tools/browser/get-content.ts:66 |
...
```ts
build(params: BrowserGetContentParams): ToolInvocation<BrowserGetContentParams, ToolFrameworkResult>;
```
...
| Parameter | Type |
| ------ | ------ |
| `params` | `BrowserGetContentParams` |

#### Returns

[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserGetContentParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserGetContentParams): ToolInvocation<BrowserGetContentParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/get-content.ts:91
...
```ts
validateBuildAndExecute(params: BrowserGetContentParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserGetContentParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: BrowserGetContentParams): string | null;
```
...
| Parameter | Type |
| ------ | ------ |
| `_params` | `BrowserGetContentParams` |
