...
# Class: BrowserGetHtmlTool

Defined in: packages/codeboltjs/src/tools/browser/get-html.ts:65

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserGetHtmlParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserGetHtmlTool(): BrowserGetHtmlTool;
```

Defined in: packages/codeboltjs/src/tools/browser/get-html.ts:68
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_get_html'` | - | - | packages/codeboltjs/src/tools/browser/get-html.ts:66 |
...
```ts
build(params: BrowserGetHtmlParams): ToolInvocation<BrowserGetHtmlParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserGetHtmlParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserGetHtmlParams): ToolInvocation<BrowserGetHtmlParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/get-html.ts:91
...
```ts
validateBuildAndExecute(params: BrowserGetHtmlParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserGetHtmlParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: BrowserGetHtmlParams): string | null;
```
