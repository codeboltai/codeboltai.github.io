...
Defined in: packages/codeboltjs/src/tools/browser/get-url.ts:65

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserGetUrlParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserGetUrlTool(): BrowserGetUrlTool;
```

Defined in: packages/codeboltjs/src/tools/browser/get-url.ts:68
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_get_url'` | - | - | packages/codeboltjs/src/tools/browser/get-url.ts:66 |
...
```ts
build(params: BrowserGetUrlParams): ToolInvocation<BrowserGetUrlParams, ToolFrameworkResult>;
```
...
| Parameter | Type |
| ------ | ------ |
| `params` | `BrowserGetUrlParams` |

#### Returns

[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserGetUrlParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserGetUrlParams): ToolInvocation<BrowserGetUrlParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/get-url.ts:91
...
```ts
validateBuildAndExecute(params: BrowserGetUrlParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserGetUrlParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: BrowserGetUrlParams): string | null;
```
