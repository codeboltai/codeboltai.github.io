...
Defined in: packages/codeboltjs/src/tools/browser/navigate.ts:66

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserNavigateParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserNavigateTool(): BrowserNavigateTool;
```

Defined in: packages/codeboltjs/src/tools/browser/navigate.ts:69
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_navigate'` | - | - | packages/codeboltjs/src/tools/browser/navigate.ts:67 |
...
```ts
build(params: BrowserNavigateParams): ToolInvocation<BrowserNavigateParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserNavigateParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserNavigateParams): ToolInvocation<BrowserNavigateParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/navigate.ts:103
...
```ts
validateBuildAndExecute(params: BrowserNavigateParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserNavigateParams): string | null;
```
...
```ts
protected validateToolParamValues(params: BrowserNavigateParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/browser/navigate.ts:96
