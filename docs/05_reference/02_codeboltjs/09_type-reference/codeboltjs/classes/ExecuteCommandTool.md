...
Defined in: packages/codeboltjs/src/tools/terminal/execute-command.ts:132

Implementation of the ExecuteCommand tool logic

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`ExecuteCommandToolParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new ExecuteCommandTool(): ExecuteCommandTool;
```

Defined in: packages/codeboltjs/src/tools/terminal/execute-command.ts:138
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'execute_command'` | - | - | packages/codeboltjs/src/tools/terminal/execute-command.ts:136 |
...
```ts
build(params: ExecuteCommandToolParams): ToolInvocation<ExecuteCommandToolParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`ExecuteCommandToolParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: ExecuteCommandToolParams): ToolInvocation<ExecuteCommandToolParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/terminal/execute-command.ts:171
...
```ts
validateBuildAndExecute(params: ExecuteCommandToolParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: ExecuteCommandToolParams): string | null;
```
...
```ts
protected validateToolParamValues(params: ExecuteCommandToolParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/terminal/execute-command.ts:161
