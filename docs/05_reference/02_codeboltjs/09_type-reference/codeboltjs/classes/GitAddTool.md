...
Defined in: packages/codeboltjs/src/tools/git/add.ts:67

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`GitAddParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new GitAddTool(): GitAddTool;
```

Defined in: packages/codeboltjs/src/tools/git/add.ts:70
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'git_add'` | - | - | packages/codeboltjs/src/tools/git/add.ts:68 |
...
```ts
build(params: GitAddParams): ToolInvocation<GitAddParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`GitAddParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: GitAddParams): ToolInvocation<GitAddParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/git/add.ts:89
...
```ts
validateBuildAndExecute(params: GitAddParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: GitAddParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: GitAddParams): string | null;
```
