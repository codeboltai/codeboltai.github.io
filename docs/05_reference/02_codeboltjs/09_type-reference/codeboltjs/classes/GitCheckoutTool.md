...
Defined in: packages/codeboltjs/src/tools/git/checkout.ts:68

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`GitCheckoutParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new GitCheckoutTool(): GitCheckoutTool;
```

Defined in: packages/codeboltjs/src/tools/git/checkout.ts:71
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'git_checkout'` | - | - | packages/codeboltjs/src/tools/git/checkout.ts:69 |
...
```ts
build(params: GitCheckoutParams): ToolInvocation<GitCheckoutParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`GitCheckoutParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: GitCheckoutParams): ToolInvocation<GitCheckoutParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/git/checkout.ts:101
...
```ts
validateBuildAndExecute(params: GitCheckoutParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: GitCheckoutParams): string | null;
```
...
```ts
protected validateToolParamValues(params: GitCheckoutParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/git/checkout.ts:94
