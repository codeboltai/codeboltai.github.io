...
Defined in: packages/codeboltjs/src/tools/git/clone.ts:73

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`GitCloneParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new GitCloneTool(): GitCloneTool;
```

Defined in: packages/codeboltjs/src/tools/git/clone.ts:76
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'git_clone'` | - | - | packages/codeboltjs/src/tools/git/clone.ts:74 |
...
```ts
build(params: GitCloneParams): ToolInvocation<GitCloneParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`GitCloneParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: GitCloneParams): ToolInvocation<GitCloneParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/git/clone.ts:110
...
```ts
validateBuildAndExecute(params: GitCloneParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: GitCloneParams): string | null;
```
...
```ts
protected validateToolParamValues(params: GitCloneParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/git/clone.ts:103
