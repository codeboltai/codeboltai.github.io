...
Defined in: packages/codeboltjs/src/tools/git/init.ts:68

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`GitInitParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new GitInitTool(): GitInitTool;
```

Defined in: packages/codeboltjs/src/tools/git/init.ts:71
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'git_init'` | - | - | packages/codeboltjs/src/tools/git/init.ts:69 |
...
```ts
build(params: GitInitParams): ToolInvocation<GitInitParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`GitInitParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: GitInitParams): ToolInvocation<GitInitParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/git/init.ts:101
...
```ts
validateBuildAndExecute(params: GitInitParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: GitInitParams): string | null;
```
...
```ts
protected validateToolParamValues(params: GitInitParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/git/init.ts:94
