...
Defined in: packages/codeboltjs/src/tools/git/push.ts:67

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`GitPushParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new GitPushTool(): GitPushTool;
```

Defined in: packages/codeboltjs/src/tools/git/push.ts:70
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'git_push'` | - | - | packages/codeboltjs/src/tools/git/push.ts:68 |
...
```ts
build(params: GitPushParams): ToolInvocation<GitPushParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`GitPushParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: GitPushParams): ToolInvocation<GitPushParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/git/push.ts:89
...
```ts
validateBuildAndExecute(params: GitPushParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: GitPushParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: GitPushParams): string | null;
```
