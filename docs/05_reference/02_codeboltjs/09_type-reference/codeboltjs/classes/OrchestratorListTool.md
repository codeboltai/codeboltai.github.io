...
# Class: OrchestratorListTool

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-list.ts:64

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`OrchestratorListParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new OrchestratorListTool(): OrchestratorListTool;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-list.ts:67
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'orchestrator_list'` | - | - | packages/codeboltjs/src/tools/orchestrator/orchestrator-list.ts:65 |
...
```ts
build(params: OrchestratorListParams): ToolInvocation<OrchestratorListParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`OrchestratorListParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: OrchestratorListParams): ToolInvocation<OrchestratorListParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-list.ts:81
...
```ts
validateBuildAndExecute(params: OrchestratorListParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: OrchestratorListParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: OrchestratorListParams): string | null;
```
...
| Parameter | Type |
| ------ | ------ |
| `_params` | `OrchestratorListParams` |
