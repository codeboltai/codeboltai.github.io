---
title: OrchestratorUpdateSettingsTool
---
...
# Class: OrchestratorUpdateSettingsTool

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-update-settings.ts:75

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`OrchestratorUpdateSettingsParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new OrchestratorUpdateSettingsTool(): OrchestratorUpdateSettingsTool;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-update-settings.ts:78
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'orchestrator_update_settings'` | - | - | packages/codeboltjs/src/tools/orchestrator/orchestrator-update-settings.ts:76 |
...
```ts
build(params: OrchestratorUpdateSettingsParams): ToolInvocation<OrchestratorUpdateSettingsParams, ToolFrameworkResult>;
```
...
| Parameter | Type |
| ------ | ------ |
| `params` | `OrchestratorUpdateSettingsParams` |

#### Returns

[`ToolInvocation`](../interfaces/ToolInvocation)\<`OrchestratorUpdateSettingsParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: OrchestratorUpdateSettingsParams): ToolInvocation<OrchestratorUpdateSettingsParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-update-settings.ts:120
...
```ts
validateBuildAndExecute(params: OrchestratorUpdateSettingsParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: OrchestratorUpdateSettingsParams): string | null;
```
...
```ts
protected validateToolParamValues(params: OrchestratorUpdateSettingsParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-update-settings.ts:113
