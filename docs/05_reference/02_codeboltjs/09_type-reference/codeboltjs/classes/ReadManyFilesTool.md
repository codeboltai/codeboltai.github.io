...
Defined in: packages/codeboltjs/src/tools/file/read-many-files.ts:189

Implementation of the ReadManyFiles tool logic

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`ReadManyFilesToolParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new ReadManyFilesTool(): ReadManyFilesTool;
```

Defined in: packages/codeboltjs/src/tools/file/read-many-files.ts:195
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'read_many_files'` | - | - | packages/codeboltjs/src/tools/file/read-many-files.ts:193 |
...
```ts
build(params: ReadManyFilesToolParams): ToolInvocation<ReadManyFilesToolParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`ReadManyFilesToolParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: ReadManyFilesToolParams): ToolInvocation<ReadManyFilesToolParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/file/read-many-files.ts:276
...
```ts
validateBuildAndExecute(params: ReadManyFilesToolParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: ReadManyFilesToolParams): string | null;
```
...
```ts
protected validateToolParamValues(params: ReadManyFilesToolParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/file/read-many-files.ts:258
