...
# Class: TaskAssignTool

Defined in: packages/codeboltjs/src/tools/task/task-assign.ts:65

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`TaskAssignParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new TaskAssignTool(): TaskAssignTool;
```

Defined in: packages/codeboltjs/src/tools/task/task-assign.ts:68
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'task_assign'` | - | - | packages/codeboltjs/src/tools/task/task-assign.ts:66 |
...
```ts
build(params: TaskAssignParams): ToolInvocation<TaskAssignParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`TaskAssignParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: TaskAssignParams): ToolInvocation<TaskAssignParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/task/task-assign.ts:105
...
```ts
validateBuildAndExecute(params: TaskAssignParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: TaskAssignParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/task/task-assign.ts:95
