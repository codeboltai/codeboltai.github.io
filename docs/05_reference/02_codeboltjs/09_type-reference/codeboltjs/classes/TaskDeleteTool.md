...
Defined in: packages/codeboltjs/src/tools/task/task-delete.ts:63

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`TaskDeleteParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new TaskDeleteTool(): TaskDeleteTool;
```

Defined in: packages/codeboltjs/src/tools/task/task-delete.ts:66
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'task_delete'` | - | - | packages/codeboltjs/src/tools/task/task-delete.ts:64 |
...
```ts
build(params: TaskDeleteParams): ToolInvocation<TaskDeleteParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`TaskDeleteParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: TaskDeleteParams): ToolInvocation<TaskDeleteParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/task/task-delete.ts:96
...
```ts
validateBuildAndExecute(params: TaskDeleteParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: TaskDeleteParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/task/task-delete.ts:89
