...
- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`TaskExecuteParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'task_execute'` | - | - | packages/codeboltjs/src/tools/task/task-execute.ts:66 |
...
```ts
build(params: TaskExecuteParams): ToolInvocation<TaskExecuteParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`TaskExecuteParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: TaskExecuteParams): ToolInvocation<TaskExecuteParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/task/task-execute.ts:105
...
```ts
validateBuildAndExecute(params: TaskExecuteParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: TaskExecuteParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/task/task-execute.ts:95
