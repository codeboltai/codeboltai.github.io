...
Defined in: packages/codeboltjs/src/tools/task/task-get.ts:75

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`TaskGetParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new TaskGetTool(): TaskGetTool;
```

Defined in: packages/codeboltjs/src/tools/task/task-get.ts:78
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'task_get'` | - | - | packages/codeboltjs/src/tools/task/task-get.ts:76 |
...
```ts
build(params: TaskGetParams): ToolInvocation<TaskGetParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`TaskGetParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: TaskGetParams): ToolInvocation<TaskGetParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/task/task-get.ts:108
...
```ts
validateBuildAndExecute(params: TaskGetParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: TaskGetParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/task/task-get.ts:101
