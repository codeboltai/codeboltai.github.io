...
# Class: ThreadCreateTool

Defined in: packages/codeboltjs/src/tools/thread/thread-create.ts:66

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`ThreadCreateParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new ThreadCreateTool(): ThreadCreateTool;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-create.ts:69
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'thread_create'` | - | - | packages/codeboltjs/src/tools/thread/thread-create.ts:67 |
...
```ts
build(params: ThreadCreateParams): ToolInvocation<ThreadCreateParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`ThreadCreateParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: ThreadCreateParams): ToolInvocation<ThreadCreateParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-create.ts:95
...
```ts
validateBuildAndExecute(params: ThreadCreateParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: ThreadCreateParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/thread/thread-create.ts:88
