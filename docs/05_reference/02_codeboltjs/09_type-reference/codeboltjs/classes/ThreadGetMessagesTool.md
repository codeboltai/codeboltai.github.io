...
# Class: ThreadGetMessagesTool

Defined in: packages/codeboltjs/src/tools/thread/thread-get-messages.ts:90

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`ThreadGetMessagesParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new ThreadGetMessagesTool(): ThreadGetMessagesTool;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-get-messages.ts:93
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'thread_get_messages'` | - | - | packages/codeboltjs/src/tools/thread/thread-get-messages.ts:91 |
...
```ts
build(params: ThreadGetMessagesParams): ToolInvocation<ThreadGetMessagesParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`ThreadGetMessagesParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: ThreadGetMessagesParams): ToolInvocation<ThreadGetMessagesParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-get-messages.ts:141
...
```ts
validateBuildAndExecute(params: ThreadGetMessagesParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: ThreadGetMessagesParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/thread/thread-get-messages.ts:128
