...
Defined in: packages/codeboltjs/src/tools/thread/thread-get.ts:65

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`ThreadGetParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new ThreadGetTool(): ThreadGetTool;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-get.ts:68
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'thread_get'` | - | - | packages/codeboltjs/src/tools/thread/thread-get.ts:66 |
...
```ts
build(params: ThreadGetParams): ToolInvocation<ThreadGetParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`ThreadGetParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: ThreadGetParams): ToolInvocation<ThreadGetParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-get.ts:94
...
```ts
validateBuildAndExecute(params: ThreadGetParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: ThreadGetParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/thread/thread-get.ts:87
