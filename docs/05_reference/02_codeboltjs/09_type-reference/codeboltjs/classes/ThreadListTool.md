...
# Class: ThreadListTool

Defined in: packages/codeboltjs/src/tools/thread/thread-list.ts:65

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`ThreadListParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new ThreadListTool(): ThreadListTool;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-list.ts:68
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'thread_list'` | - | - | packages/codeboltjs/src/tools/thread/thread-list.ts:66 |
...
```ts
build(params: ThreadListParams): ToolInvocation<ThreadListParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`ThreadListParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: ThreadListParams): ToolInvocation<ThreadListParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-list.ts:94
...
```ts
validateBuildAndExecute(params: ThreadListParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: ThreadListParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/thread/thread-list.ts:87
