...
# Class: ThreadUpdateStatusTool

Defined in: packages/codeboltjs/src/tools/thread/thread-update-status.ts:65

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`ThreadUpdateStatusParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new ThreadUpdateStatusTool(): ThreadUpdateStatusTool;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-update-status.ts:68
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'thread_update_status'` | - | - | packages/codeboltjs/src/tools/thread/thread-update-status.ts:66 |
...
```ts
build(params: ThreadUpdateStatusParams): ToolInvocation<ThreadUpdateStatusParams, ToolFrameworkResult>;
```
...
| Parameter | Type |
| ------ | ------ |
| `params` | `ThreadUpdateStatusParams` |

#### Returns

[`ToolInvocation`](../interfaces/ToolInvocation)\<`ThreadUpdateStatusParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: ThreadUpdateStatusParams): ToolInvocation<ThreadUpdateStatusParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-update-status.ts:101
...
```ts
validateBuildAndExecute(params: ThreadUpdateStatusParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: ThreadUpdateStatusParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/thread/thread-update-status.ts:91
