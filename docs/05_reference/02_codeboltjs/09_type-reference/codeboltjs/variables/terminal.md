...
| ------ | ------ | ------ | ------ |
| <a id="eventemitter"></a> `eventEmitter` | `CustomEventEmitter` | - | packages/codeboltjs/src/modules/terminal.ts:31 |
...
| `executeCommandWithStream()` | (`command`: `string`, `executeInMain`: `boolean`) => `CustomEventEmitter` | Executes a given command and streams the output. Listens for messages from the WebSocket and streams the output data. | packages/codeboltjs/src/modules/terminal.ts:111 |
