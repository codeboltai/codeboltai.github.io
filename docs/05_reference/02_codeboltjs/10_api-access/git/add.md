...
```typescript
codebolt.git.addAll(): Promise<AddResponse>
```
...
- **`Promise<AddResponse>`**: A promise that resolves with an [`AddResponse`](/docs/reference/codeboltjs/type-reference/types/interfaces/AddResponse) object containing the response from the add operation.

### Response Structure

The method returns a Promise that resolves to an [`AddResponse`](/docs/reference/codeboltjs/type-reference/types/interfaces/AddResponse) object with the following properties:
