...
cbparameters:
  ...
  returns:
    signatureTypeName: "Promise<{ completion: LLMCompletion }>"
    description: "A promise that resolves with an object containing the LLM's completion with content, token usage, and completion details."
data:
  name: inference
  category: llm
  link: inference.md
...
```typescript
codebolt.llm.inference(params: LLMInferenceParams): Promise<{ completion: LLMCompletion }>
```

Sends an inference request to the LLM using OpenAI message format with tools support. The model is selected based on the provided llmrole parameter. If the specific model for the role is not found, it falls back to the default model for the current agent, and ultimately to the default application-wide LLM if necessary.
### Parameters

- **`message`** ([LLMInferenceParams](/docs/reference/codeboltjs/type-reference/codeboltjs/interfaces/LLMInferenceParams)): The inference parameters object containing messages, tools, and configuration options.
- **`message.messages`** ([Message](/docs/reference/codeboltjs/type-reference/codeboltjs/interfaces/Message)[]): Array of conversation messages with roles ('user', 'assistant', 'tool', 'system') and content.
- **`message.tools`** ([Tool](/docs/reference/codeboltjs/type-reference/codeboltjs/interfaces/Tool)[]): Optional: Available tools for the model to use. Each tool has a type and function definition.
...
- **`Promise<{ completion: LLMCompletion }>`**: A promise that resolves with an object containing the LLM's completion with content, token usage, and completion details.

### Response Structure

The method returns a Promise that resolves to an object with a `completion` property. Access response data via `response.completion`:

- **`completion.content`** (string): The actual response content from the LLM.
- **`completion.role`** (string): Always "assistant" for LLM responses.
- **`completion.model`** (string, optional): The specific model used for the inference.
- **`completion.usage`** (object, optional): Token usage information including:
  - **`prompt_tokens`** (number): Number of tokens in the prompt.
  - **`completion_tokens`** (number): Number of tokens in the completion.
  - **`total_tokens`** (number): Total tokens used (prompt + completion).
- **`completion.finish_reason`** (string, optional): Reason why the model stopped generating (e.g., "stop", "length", "tool_calls").
- **`completion.choices`** (array, optional): Array of completion choices with message and finish_reason.
- **`completion.success`** (boolean, optional): Indicates if the operation was successful.
- **`completion.message`** (string, optional): Additional information about the response.
- **`completion.error`** (string, optional): Error details if the operation failed.
- **`completion.messageId`** (string, optional): Unique identifier for the message.
- **`completion.threadId`** (string, optional): Thread identifier for conversation context.
...
console.log("Code Response:", conversationResponse.completion.content);
...
console.log("Tool Call Response:", toolCallResponse.completion.content);
...
console.log("Complex Response:", complexResponse.completion.content);
```
