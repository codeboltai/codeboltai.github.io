...
- **`data`** ([CreateFeatureData](/docs/reference/codeboltjs/type-reference/codeboltjs/interfaces/CreateFeatureData)): Feature data including title, description, and optional metadata
- **`projectPath`** (string): Optional project path (uses active project if not provided)

### Returns

- **`Promise<[RoadmapFeatureResponse](/docs/reference/codeboltjs/type-reference/codeboltjs/interfaces/RoadmapFeatureResponse)>`**: A promise that resolves to the created feature
