...
# Class: BackgroundAgentSocket

Defined in: CodeBolt/packages/pluginSdk/src/sockets/background-agent.socket.ts:9

Background Agent WebSocket - Monitor background agent processes.
Type-based protocol with subscribe for targeted updates and broadcast events.

## Extends

- [`TypedEventEmitter`](TypedEventEmitter)\<`BackgroundAgentSocketEvents`\>
...
```ts
new BackgroundAgentSocket(connectionManager: SocketConnectionManager): BackgroundAgentSocket;
```

Defined in: CodeBolt/packages/pluginSdk/src/sockets/background-agent.socket.ts:12
...
```ts
emit<K>(event: K, ...args: Parameters<BackgroundAgentSocketEvents[K]>): boolean;
```
...
| ------ | ------ |
| `event` | `K` |
| ...`args` | `Parameters`\<`BackgroundAgentSocketEvents`\[`K`\]\> |
...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/background-agent.socket.ts:46
...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/background-agent.socket.ts:43
...
```ts
off<K>(event: K, listener: BackgroundAgentSocketEvents[K]): this;
```
...
| ------ | ------ |
| `event` | `K` |
| `listener` | `BackgroundAgentSocketEvents`\[`K`\] |
...
```ts
on<K>(event: K, listener: BackgroundAgentSocketEvents[K]): this;
```
...
```ts
once<K>(event: K, listener: BackgroundAgentSocketEvents[K]): this;
```
...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/background-agent.socket.ts:52
...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/background-agent.socket.ts:49
