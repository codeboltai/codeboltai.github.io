...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/browser.socket.ts:9

Browser WebSocket - Send browser events and receive responses.
Events: htmlReceived, clickResponse, scrollResponse, typeResponse, etc.

## Extends

- [`TypedEventEmitter`](TypedEventEmitter)\<`BrowserSocketEvents`\>
...
```ts
new BrowserSocket(connectionManager: SocketConnectionManager, socketName: string): BrowserSocket;
```

Defined in: CodeBolt/packages/pluginSdk/src/sockets/browser.socket.ts:12
...
```ts
emit<K>(event: K, ...args: Parameters<BrowserSocketEvents[K]>): boolean;
```
...
| ------ | ------ |
| `event` | `K` |
| ...`args` | `Parameters`\<`BrowserSocketEvents`\[`K`\]\> |
...
```ts
off<K>(event: K, listener: BrowserSocketEvents[K]): this;
```
...
| ------ | ------ |
| `event` | `K` |
| `listener` | `BrowserSocketEvents`\[`K`\] |
...
```ts
on<K>(event: K, listener: BrowserSocketEvents[K]): this;
```
...
```ts
once<K>(event: K, listener: BrowserSocketEvents[K]): this;
```
...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/browser.socket.ts:49
