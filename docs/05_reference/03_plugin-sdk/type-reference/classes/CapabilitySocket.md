...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/capability.socket.ts:9

Capability WebSocket - Query and monitor capabilities, executors, and executions.
Action-based protocol with broadcast events.

## Extends

- [`TypedEventEmitter`](TypedEventEmitter)\<`CapabilitySocketEvents`\>
...
```ts
new CapabilitySocket(connectionManager: SocketConnectionManager, socketName: string): CapabilitySocket;
```

Defined in: CodeBolt/packages/pluginSdk/src/sockets/capability.socket.ts:12
...
```ts
emit<K>(event: K, ...args: Parameters<CapabilitySocketEvents[K]>): boolean;
```
...
| ------ | ------ |
| `event` | `K` |
| ...`args` | `Parameters`\<`CapabilitySocketEvents`\[`K`\]\> |
...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/capability.socket.ts:118
...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/capability.socket.ts:106
...
```ts
getCapabilityTypes(): void;
```

Defined in: CodeBolt/packages/pluginSdk/src/sockets/capability.socket.ts:112
...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/capability.socket.ts:109
...
```ts
off<K>(event: K, listener: CapabilitySocketEvents[K]): this;
```
...
| ------ | ------ |
| `event` | `K` |
| `listener` | `CapabilitySocketEvents`\[`K`\] |
...
```ts
on<K>(event: K, listener: CapabilitySocketEvents[K]): this;
```
...
```ts
once<K>(event: K, listener: CapabilitySocketEvents[K]): this;
```
...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/capability.socket.ts:115
