...
# Class: ContextAssemblySocket

Defined in: CodeBolt/packages/pluginSdk/src/sockets/context-assembly.socket.ts:9

Context Assembly WebSocket - Receive context assembly pipeline broadcast events.
Broadcast-only: started, rulesEvaluated, pipelineComplete, complete, error.

## Extends

- [`TypedEventEmitter`](TypedEventEmitter)\<`ContextAssemblySocketEvents`\>
...
```ts
new ContextAssemblySocket(connectionManager: SocketConnectionManager): ContextAssemblySocket;
```

Defined in: CodeBolt/packages/pluginSdk/src/sockets/context-assembly.socket.ts:12
...
```ts
emit<K>(event: K, ...args: Parameters<ContextAssemblySocketEvents[K]>): boolean;
```
...
| ------ | ------ |
| `event` | `K` |
| ...`args` | `Parameters`\<`ContextAssemblySocketEvents`\[`K`\]\> |
...
```ts
off<K>(event: K, listener: ContextAssemblySocketEvents[K]): this;
```
...
| ------ | ------ |
| `event` | `K` |
| `listener` | `ContextAssemblySocketEvents`\[`K`\] |
...
```ts
on<K>(event: K, listener: ContextAssemblySocketEvents[K]): this;
```
...
```ts
once<K>(event: K, listener: ContextAssemblySocketEvents[K]): this;
```
...
Defined in: CodeBolt/packages/pluginSdk/src/sockets/context-assembly.socket.ts:38
