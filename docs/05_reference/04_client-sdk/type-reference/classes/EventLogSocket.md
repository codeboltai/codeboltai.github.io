...
# Class: EventLogSocket

Defined in: CodeBolt/packages/clientsdk/src/sockets/event-log.socket.ts:9

Event Log WebSocket - Receive event log broadcast events.
Broadcast-only: instance CRUD, event appended.

## Extends

- [`TypedEventEmitter`](TypedEventEmitter)\<`EventLogSocketEvents`\>
...
```ts
new EventLogSocket(connectionManager: SocketConnectionManager, socketName: string): EventLogSocket;
```

Defined in: CodeBolt/packages/clientsdk/src/sockets/event-log.socket.ts:12
...
```ts
emit<K>(event: K, ...args: Parameters<EventLogSocketEvents[K]>): boolean;
```
...
| ------ | ------ |
| `event` | `K` |
| ...`args` | `Parameters`\<`EventLogSocketEvents`\[`K`\]\> |
...
```ts
off<K>(event: K, listener: EventLogSocketEvents[K]): this;
```
...
| ------ | ------ |
| `event` | `K` |
| `listener` | `EventLogSocketEvents`\[`K`\] |
...
```ts
on<K>(event: K, listener: EventLogSocketEvents[K]): this;
```
...
```ts
once<K>(event: K, listener: EventLogSocketEvents[K]): this;
```
...
Defined in: CodeBolt/packages/clientsdk/src/sockets/event-log.socket.ts:78
...
Defined in: CodeBolt/packages/clientsdk/src/sockets/event-log.socket.ts:81
