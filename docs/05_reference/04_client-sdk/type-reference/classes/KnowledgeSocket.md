...
# Class: KnowledgeSocket

Defined in: CodeBolt/packages/clientsdk/src/sockets/knowledge.socket.ts:9

Knowledge WebSocket - Receive knowledge base broadcast events.
Broadcast-only: collections, documents, chunking progress.

## Extends

- [`TypedEventEmitter`](TypedEventEmitter)\<`KnowledgeSocketEvents`\>
...
```ts
new KnowledgeSocket(connectionManager: SocketConnectionManager): KnowledgeSocket;
```

Defined in: CodeBolt/packages/clientsdk/src/sockets/knowledge.socket.ts:12
...
```ts
emit<K>(event: K, ...args: Parameters<KnowledgeSocketEvents[K]>): boolean;
```
...
| ------ | ------ |
| `event` | `K` |
| ...`args` | `Parameters`\<`KnowledgeSocketEvents`\[`K`\]\> |
...
```ts
off<K>(event: K, listener: KnowledgeSocketEvents[K]): this;
```
...
| ------ | ------ |
| `event` | `K` |
| `listener` | `KnowledgeSocketEvents`\[`K`\] |
...
```ts
on<K>(event: K, listener: KnowledgeSocketEvents[K]): this;
```
...
```ts
once<K>(event: K, listener: KnowledgeSocketEvents[K]): this;
```
...
Defined in: CodeBolt/packages/clientsdk/src/sockets/knowledge.socket.ts:43
...
Defined in: CodeBolt/packages/clientsdk/src/sockets/knowledge.socket.ts:46
