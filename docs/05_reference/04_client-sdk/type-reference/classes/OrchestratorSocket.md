...
Defined in: CodeBolt/packages/clientsdk/src/sockets/orchestrator.socket.ts:10

Orchestrator WebSocket - Subscribe to orchestrator thread/message updates.
Subscribe-based: subscribe/unsubscribe for targeted updates.

## Extends

- [`TypedEventEmitter`](TypedEventEmitter)\<`OrchestratorSocketEvents`\>
...
```ts
new OrchestratorSocket(connectionManager: SocketConnectionManager): OrchestratorSocket;
```

Defined in: CodeBolt/packages/clientsdk/src/sockets/orchestrator.socket.ts:14
...
```ts
emit<K>(event: K, ...args: Parameters<OrchestratorSocketEvents[K]>): boolean;
```
...
| ------ | ------ |
| `event` | `K` |
| ...`args` | `Parameters`\<`OrchestratorSocketEvents`\[`K`\]\> |
...
```ts
off<K>(event: K, listener: OrchestratorSocketEvents[K]): this;
```
...
| ------ | ------ |
| `event` | `K` |
| `listener` | `OrchestratorSocketEvents`\[`K`\] |
...
```ts
on<K>(event: K, listener: OrchestratorSocketEvents[K]): this;
```
...
```ts
once<K>(event: K, listener: OrchestratorSocketEvents[K]): this;
```
...
Defined in: CodeBolt/packages/clientsdk/src/sockets/orchestrator.socket.ts:65
...
Defined in: CodeBolt/packages/clientsdk/src/sockets/orchestrator.socket.ts:59
...
Defined in: CodeBolt/packages/clientsdk/src/sockets/orchestrator.socket.ts:62
