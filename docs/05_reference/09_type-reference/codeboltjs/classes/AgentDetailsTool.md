...
# Class: AgentDetailsTool

Defined in: packages/codeboltjs/src/tools/agent/agent-details.ts:90

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`AgentDetailsParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new AgentDetailsTool(): AgentDetailsTool;
```

Defined in: packages/codeboltjs/src/tools/agent/agent-details.ts:93
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'agent_details'` | - | - | packages/codeboltjs/src/tools/agent/agent-details.ts:91 |
...
```ts
build(params: AgentDetailsParams): ToolInvocation<AgentDetailsParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`AgentDetailsParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: AgentDetailsParams): ToolInvocation<AgentDetailsParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/agent/agent-details.ts:117
...
```ts
validateBuildAndExecute(params: AgentDetailsParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: AgentDetailsParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: AgentDetailsParams): string | null;
```
