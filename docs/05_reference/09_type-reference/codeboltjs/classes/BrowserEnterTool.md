...
# Class: BrowserEnterTool

Defined in: packages/codeboltjs/src/tools/browser/enter.ts:64

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserEnterParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserEnterTool(): BrowserEnterTool;
```

Defined in: packages/codeboltjs/src/tools/browser/enter.ts:67
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_enter'` | - | - | packages/codeboltjs/src/tools/browser/enter.ts:65 |
...
```ts
build(params: BrowserEnterParams): ToolInvocation<BrowserEnterParams, ToolFrameworkResult>;
```
...
| Parameter | Type |
| ------ | ------ |
| `params` | `BrowserEnterParams` |

#### Returns

[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserEnterParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserEnterParams): ToolInvocation<BrowserEnterParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/enter.ts:90
...
```ts
validateBuildAndExecute(params: BrowserEnterParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserEnterParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: BrowserEnterParams): string | null;
```
