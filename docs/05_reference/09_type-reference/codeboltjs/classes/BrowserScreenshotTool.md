...
Defined in: packages/codeboltjs/src/tools/browser/screenshot.ts:80

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserScreenshotParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserScreenshotTool(): BrowserScreenshotTool;
```

Defined in: packages/codeboltjs/src/tools/browser/screenshot.ts:83
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_screenshot'` | - | - | packages/codeboltjs/src/tools/browser/screenshot.ts:81 |
...
```ts
build(params: BrowserScreenshotParams): ToolInvocation<BrowserScreenshotParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserScreenshotParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserScreenshotParams): ToolInvocation<BrowserScreenshotParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/screenshot.ts:119
...
```ts
validateBuildAndExecute(params: BrowserScreenshotParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserScreenshotParams): string | null;
```
...
```ts
protected validateToolParamValues(_params: BrowserScreenshotParams): string | null;
```
...
| Parameter | Type |
| ------ | ------ |
| `_params` | `BrowserScreenshotParams` |
