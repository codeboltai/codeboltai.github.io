...
Defined in: packages/codeboltjs/src/tools/browser/scroll.ts:68

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserScrollParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserScrollTool(): BrowserScrollTool;
```

Defined in: packages/codeboltjs/src/tools/browser/scroll.ts:71
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_scroll'` | - | - | packages/codeboltjs/src/tools/browser/scroll.ts:69 |
...
```ts
build(params: BrowserScrollParams): ToolInvocation<BrowserScrollParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserScrollParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserScrollParams): ToolInvocation<BrowserScrollParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/scroll.ts:116
...
```ts
validateBuildAndExecute(params: BrowserScrollParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserScrollParams): string | null;
```
...
```ts
protected validateToolParamValues(params: BrowserScrollParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/browser/scroll.ts:103
