...
Defined in: packages/codeboltjs/src/tools/browser/search.ts:68

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserSearchParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserSearchTool(): BrowserSearchTool;
```

Defined in: packages/codeboltjs/src/tools/browser/search.ts:71
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_search'` | - | - | packages/codeboltjs/src/tools/browser/search.ts:69 |
...
```ts
build(params: BrowserSearchParams): ToolInvocation<BrowserSearchParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserSearchParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserSearchParams): ToolInvocation<BrowserSearchParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/search.ts:112
...
```ts
validateBuildAndExecute(params: BrowserSearchParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserSearchParams): string | null;
```
...
```ts
protected validateToolParamValues(params: BrowserSearchParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/browser/search.ts:102
