...
Defined in: packages/codeboltjs/src/tools/browser/type.ts:68

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`BrowserTypeParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new BrowserTypeTool(): BrowserTypeTool;
```

Defined in: packages/codeboltjs/src/tools/browser/type.ts:71
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'browser_type'` | - | - | packages/codeboltjs/src/tools/browser/type.ts:69 |
...
```ts
build(params: BrowserTypeParams): ToolInvocation<BrowserTypeParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`BrowserTypeParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: BrowserTypeParams): ToolInvocation<BrowserTypeParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/browser/type.ts:112
...
```ts
validateBuildAndExecute(params: BrowserTypeParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: BrowserTypeParams): string | null;
```
...
```ts
protected validateToolParamValues(params: BrowserTypeParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/browser/type.ts:102
