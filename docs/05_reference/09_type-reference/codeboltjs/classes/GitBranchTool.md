...
Defined in: packages/codeboltjs/src/tools/git/branch.ts:68

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`GitBranchParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new GitBranchTool(): GitBranchTool;
```

Defined in: packages/codeboltjs/src/tools/git/branch.ts:71
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'git_branch'` | - | - | packages/codeboltjs/src/tools/git/branch.ts:69 |
...
```ts
build(params: GitBranchParams): ToolInvocation<GitBranchParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`GitBranchParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: GitBranchParams): ToolInvocation<GitBranchParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/git/branch.ts:101
...
```ts
validateBuildAndExecute(params: GitBranchParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
protected validateToolParamValues(params: GitBranchParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/git/branch.ts:94
