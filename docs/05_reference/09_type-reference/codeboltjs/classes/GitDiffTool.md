...
Defined in: packages/codeboltjs/src/tools/git/diff.ts:71

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`GitDiffParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new GitDiffTool(): GitDiffTool;
```

Defined in: packages/codeboltjs/src/tools/git/diff.ts:74
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'git_diff'` | - | - | packages/codeboltjs/src/tools/git/diff.ts:72 |
...
```ts
build(params: GitDiffParams): ToolInvocation<GitDiffParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`GitDiffParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: GitDiffParams): ToolInvocation<GitDiffParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/git/diff.ts:104
...
```ts
validateBuildAndExecute(params: GitDiffParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
protected validateToolParamValues(params: GitDiffParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/git/diff.ts:97
