...
Defined in: packages/codeboltjs/src/tools/git/logs.ts:83

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`GitLogsParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new GitLogsTool(): GitLogsTool;
```

Defined in: packages/codeboltjs/src/tools/git/logs.ts:86
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'git_logs'` | - | - | packages/codeboltjs/src/tools/git/logs.ts:84 |
...
```ts
build(params: GitLogsParams): ToolInvocation<GitLogsParams, ToolFrameworkResult>;
```
...
| Parameter | Type |
| ------ | ------ |
| `params` | `GitLogsParams` |

#### Returns

[`ToolInvocation`](../interfaces/ToolInvocation)\<`GitLogsParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: GitLogsParams): ToolInvocation<GitLogsParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/git/logs.ts:116
...
```ts
validateBuildAndExecute(params: GitLogsParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: GitLogsParams): string | null;
```
...
```ts
protected validateToolParamValues(params: GitLogsParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/git/logs.ts:109
