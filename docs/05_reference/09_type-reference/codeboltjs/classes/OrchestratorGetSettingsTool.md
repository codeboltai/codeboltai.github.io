...
# Class: OrchestratorGetSettingsTool

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-get-settings.ts:61

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`OrchestratorGetSettingsParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new OrchestratorGetSettingsTool(): OrchestratorGetSettingsTool;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-get-settings.ts:64
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'orchestrator_get_settings'` | - | - | packages/codeboltjs/src/tools/orchestrator/orchestrator-get-settings.ts:62 |
...
```ts
build(params: OrchestratorGetSettingsParams): ToolInvocation<OrchestratorGetSettingsParams, ToolFrameworkResult>;
```
...
| Parameter | Type |
| ------ | ------ |
| `params` | `OrchestratorGetSettingsParams` |

#### Returns

[`ToolInvocation`](../interfaces/ToolInvocation)\<`OrchestratorGetSettingsParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: OrchestratorGetSettingsParams): ToolInvocation<OrchestratorGetSettingsParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-get-settings.ts:90
...
```ts
validateBuildAndExecute(params: OrchestratorGetSettingsParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: OrchestratorGetSettingsParams): string | null;
```
...
```ts
protected validateToolParamValues(params: OrchestratorGetSettingsParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-get-settings.ts:83
