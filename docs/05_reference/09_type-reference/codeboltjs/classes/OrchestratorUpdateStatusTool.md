...
# Class: OrchestratorUpdateStatusTool

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-update-status.ts:67

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`OrchestratorUpdateStatusParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new OrchestratorUpdateStatusTool(): OrchestratorUpdateStatusTool;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-update-status.ts:70
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'orchestrator_update_status'` | - | - | packages/codeboltjs/src/tools/orchestrator/orchestrator-update-status.ts:68 |
...
```ts
build(params: OrchestratorUpdateStatusParams): ToolInvocation<OrchestratorUpdateStatusParams, ToolFrameworkResult>;
```
...
| Parameter | Type |
| ------ | ------ |
| `params` | `OrchestratorUpdateStatusParams` |

#### Returns

[`ToolInvocation`](../interfaces/ToolInvocation)\<`OrchestratorUpdateStatusParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: OrchestratorUpdateStatusParams): ToolInvocation<OrchestratorUpdateStatusParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-update-status.ts:108
...
```ts
validateBuildAndExecute(params: OrchestratorUpdateStatusParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: OrchestratorUpdateStatusParams): string | null;
```
...
```ts
protected validateToolParamValues(params: OrchestratorUpdateStatusParams): string | null;
```

Defined in: packages/codeboltjs/src/tools/orchestrator/orchestrator-update-status.ts:94
