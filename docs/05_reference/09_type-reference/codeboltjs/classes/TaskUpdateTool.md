...
Defined in: packages/codeboltjs/src/tools/task/task-update.ts:74

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`TaskUpdateParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new TaskUpdateTool(): TaskUpdateTool;
```

Defined in: packages/codeboltjs/src/tools/task/task-update.ts:77
...
```ts
build(params: TaskUpdateParams): ToolInvocation<TaskUpdateParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`TaskUpdateParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: TaskUpdateParams): ToolInvocation<TaskUpdateParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/task/task-update.ts:119
...
```ts
validateBuildAndExecute(params: TaskUpdateParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: TaskUpdateParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/task/task-update.ts:112
