...
# Class: ThreadCreateStartTool

Defined in: packages/codeboltjs/src/tools/thread/thread-create-start.ts:66

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`ThreadCreateStartParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new ThreadCreateStartTool(): ThreadCreateStartTool;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-create-start.ts:69
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'thread_create_start'` | - | - | packages/codeboltjs/src/tools/thread/thread-create-start.ts:67 |
...
```ts
build(params: ThreadCreateStartParams): ToolInvocation<ThreadCreateStartParams, ToolFrameworkResult>;
```
...
| Parameter | Type |
| ------ | ------ |
| `params` | `ThreadCreateStartParams` |

#### Returns

[`ToolInvocation`](../interfaces/ToolInvocation)\<`ThreadCreateStartParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: ThreadCreateStartParams): ToolInvocation<ThreadCreateStartParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-create-start.ts:95
...
```ts
validateBuildAndExecute(params: ThreadCreateStartParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: ThreadCreateStartParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/thread/thread-create-start.ts:88
