...
# Class: ThreadDeleteTool

Defined in: packages/codeboltjs/src/tools/thread/thread-delete.ts:57

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`ThreadDeleteParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new ThreadDeleteTool(): ThreadDeleteTool;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-delete.ts:60
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'thread_delete'` | - | - | packages/codeboltjs/src/tools/thread/thread-delete.ts:58 |
...
```ts
build(params: ThreadDeleteParams): ToolInvocation<ThreadDeleteParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`ThreadDeleteParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: ThreadDeleteParams): ToolInvocation<ThreadDeleteParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-delete.ts:86
...
```ts
validateBuildAndExecute(params: ThreadDeleteParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: ThreadDeleteParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/thread/thread-delete.ts:79
