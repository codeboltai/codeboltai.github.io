...
# Class: ThreadUpdateTool

Defined in: packages/codeboltjs/src/tools/thread/thread-update.ts:65

Base class for declarative tools with built-in validation

## Extends

- [`BaseDeclarativeTool`](BaseDeclarativeTool)\<`ThreadUpdateParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
new ThreadUpdateTool(): ThreadUpdateTool;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-update.ts:68
...
| <a id="name-1"></a> `Name` | `readonly` | `string` | `'thread_update'` | - | - | packages/codeboltjs/src/tools/thread/thread-update.ts:66 |
...
```ts
build(params: ThreadUpdateParams): ToolInvocation<ThreadUpdateParams, ToolFrameworkResult>;
```
...
[`ToolInvocation`](../interfaces/ToolInvocation)\<`ThreadUpdateParams`, [`ToolFrameworkResult`](../interfaces/ToolFrameworkResult)\>
...
```ts
protected createInvocation(params: ThreadUpdateParams): ToolInvocation<ThreadUpdateParams, ToolFrameworkResult>;
```

Defined in: packages/codeboltjs/src/tools/thread/thread-update.ts:101
...
```ts
validateBuildAndExecute(params: ThreadUpdateParams, abortSignal: AbortSignal): Promise<ToolFrameworkResult>;
```
...
```ts
validateToolParams(params: ThreadUpdateParams): string | null;
```
...
Defined in: packages/codeboltjs/src/tools/thread/thread-update.ts:91
