---
title: BaseVectorDBSDKResponse
---

[**@codebolt/types**](../index.md)

***

# Interface: BaseVectorDBSDKResponse

Defined in: common/types/src/codeboltjstypes/libFunctionTypes/vectordb.ts:7

Vector Database SDK Function Types
Types for the cbvectordb module functions

## Extended by

- [`AddVectorItemResponse`](AddVectorItemResponse)
- [`GetVectorResponse`](GetVectorResponse)
- [`QueryVectorItemResponse`](QueryVectorItemResponse)
- [`QueryVectorItemsResponse`](QueryVectorItemsResponse)

## Properties

| Property | Type | Defined in |
| ------ | ------ | ------ |
| <a id="error"></a> `error?` | `string` | common/types/src/codeboltjstypes/libFunctionTypes/vectordb.ts:10 |
| <a id="message"></a> `message?` | `string` | common/types/src/codeboltjstypes/libFunctionTypes/vectordb.ts:9 |
| <a id="success"></a> `success?` | `boolean` | common/types/src/codeboltjstypes/libFunctionTypes/vectordb.ts:8 |
